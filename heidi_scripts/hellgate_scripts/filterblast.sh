...
#apptainer exec --bind /scratch/$USER/ ${blast} blastdbcmd -entry_batch seqIDlist.txt -db scaffolds.fasta -target_only -out ${prefix}_scaf_filtered.fasta

#apptainer exec --bind /scratch/$USER/ ${quast} quast.py ${prefix}_scaf_filtered.fasta -o quast/
...
apptainer exec --bind /scratch/$USER/ --bind /projects/$USER/ ${blast} blastn -db ${SBdb} -query ${prefix}_scaf_filtered.fasta -out localblast/${prefix}_blast_SB.txt -outfmt 6 -max_hsps 1

apptainer exec --bind /scratch/$USER/ --bind /projects/$USER/ ${blast} blastn -db ${cpdb} -query ${prefix}_scaf_filtered.fasta -out localblast/${prefix}_blast_cp.txt -outfmt 6 -max_hsps 1

apptainer exec --bind /scratch/$USER/ --bind /projects/$USER/ ${blast} blastn -db ${mtdb} -query ${prefix}_scaf_filtered.fasta -out localblast/${prefix}_blast_mt.txt -outfmt 6 -max_hsps 1
