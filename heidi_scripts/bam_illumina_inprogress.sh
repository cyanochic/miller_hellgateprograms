...
#apptainer commands
APPTAINER="apptainer exec --bind /scratch/netID"

#INPUTS (DO NOT CHANGE/ INPUT WHEN ENTERING COMMAND OR IN SBATCH SCRIPT)
...
#apptainer exec --bind /scratch/netID/ ~/bin/bwa.sif bwa aln -t 16 ${PREFIX} ${FWD} > R1.bwa
#apptainer exec --bind /scratch/netID/ ~/bin/bwa.sif bwa aln -t 16 ${PREFIX} ${RVS} > R2.bwa
